...
//
      ...
      {
          $this->nm_tira_formatacao();
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_Login']['dados_select']['links']) && !isset($this->nmgp_refresh_fields))
          { 
              $this->links = $_SESSION['sc_session'][$this->Ini->sc_page]['sec_Login']['dados_select']['links'];
          } 
...
if (!isset($this->sc_temp_USUARIO_NAVIERA)) {$this->sc_temp_USUARIO_NAVIERA = (isset($_SESSION['USUARIO_NAVIERA'])) ? $_SESSION['USUARIO_NAVIERA'] : "";}
...
if (!isset($this->sc_temp_PREDIO03_YARD)) {$this->sc_temp_PREDIO03_YARD = (isset($_SESSION['PREDIO03_YARD'])) ? $_SESSION['PREDIO03_YARD'] : "";}
...
if (!isset($this->sc_temp_Naviera_Descripcion)) {$this->sc_temp_Naviera_Descripcion = (isset($_SESSION['Naviera_Descripcion'])) ? $_SESSION['Naviera_Descripcion'] : "";}
if (!isset($this->sc_temp_yarda_desc)) {$this->sc_temp_yarda_desc = (isset($_SESSION['yarda_desc'])) ? $_SESSION['yarda_desc'] : "";}
  ...
  if($this->sc_logged_is_blocked()) { if (isset($this->Campos_Mens_erro) && !empty($this->Campos_Mens_erro))
{
 ...
 if (isset($this->sc_temp_yarda_desc)) { $_SESSION['yarda_desc'] = $this->sc_temp_yarda_desc;}
 if (isset($this->sc_temp_Naviera_Descripcion)) { $_SESSION['Naviera_Descripcion'] = $this->sc_temp_Naviera_Descripcion;}
 ...
 if (isset($this->sc_temp_PREDIO03_YARD)) { $_SESSION['PREDIO03_YARD'] = $this->sc_temp_PREDIO03_YARD;}
 ...
 if (isset($this->sc_temp_USUARIO_NAVIERA)) { $_SESSION['USUARIO_NAVIERA'] = $this->sc_temp_USUARIO_NAVIERA;}
...
$sql = "SELECT 
		...
		sec_users.idyarda,
		sec_users.idNaviera,
		sec_users.idtipo_usuario,
		yarda.descripcion_yarda
	      naviera.descripcion_larga
	      FROM sec_users, yarda, naviera
	      WHERE sec_users.idyarda=yarda.idyarda 
		  AND sec_users.idNaviera=naviera.idNaviera		  
		  AND login = $slogin
		AND pswd = ".$spswd."";
...
else if($this->rs[0][1] == 'Y')
{
	...
	$idyarda		= $this->rs[0][4];
	$idNaviera		= $this->rs[0][5];
	$idtipo_usuario		= $this->rs[0][6];
	$yarda_desc		= $this->rs[0][7];
	$Naviera_Descripcion=$this->rs[0][8];
	$DESPACHO = "2";
	$INGRESO = "1";
...
;
	 if (isset($idyarda)) {$this->sc_temp_idyarda = $idyarda;}
;
	 if (isset($idNaviera)) {$this->sc_temp_idNaviera = $idNaviera;}
;
	 if (isset($idtipo_usuario)) {$this->sc_temp_idtipo_usuario = $idtipo_usuario;}
;
	 if (isset($yarda_desc)) {$this->sc_temp_yarda_desc = $yarda_desc;}
;
	 if (isset($Naviera_Descripcion)) {$this->sc_temp_Naviera_Descripcion = $Naviera_Descripcion;}
;
	 if (isset($DESPACHO)) {$this->sc_temp_DESPACHO = $DESPACHO;}
;
	 if (isset($INGRESO)) {$this->sc_temp_INGRESO = $INGRESO;}
;
	
	 if (isset($VIGENTE)) {$this->sc_temp_VIGENTE = $VIGENTE;}
;
	 if (isset($NO_VIGENTE)) {$this->sc_temp_NO_VIGENTE = $NO_VIGENTE;}
;
	 if (isset($DEPURADO)) {$this->sc_temp_DEPURADO = $DEPURADO;}
;
	
	 if (isset($BARRIOS_YARD)) {$this->sc_temp_BARRIOS_YARD = $BARRIOS_YARD;}
;	
	 if (isset($PREPUERTO_YARD)) {$this->sc_temp_PREPUERTO_YARD = $PREPUERTO_YARD;}
;
	 if (isset($PREDIO03_YARD)) {$this->sc_temp_PREDIO03_YARD = $PREDIO03_YARD;}
;
	
	 if (isset($USUARIO_YARDA)) {$this->sc_temp_USUARIO_YARDA = $USUARIO_YARDA;}
;
	 if (isset($USUARIO_MASTER)) {$this->sc_temp_USUARIO_MASTER = $USUARIO_MASTER;}
;
	 if (isset($USUARIO_NAVIERA)) {$this->sc_temp_USUARIO_NAVIERA = $USUARIO_NAVIERA;}
;
	 if (isset($USUARIO_BODEGA)) {$this->sc_temp_USUARIO_BODEGA = $USUARIO_BODEGA;}
;
	
	$codigo='0';
 if (isset($codigo)) {$this->sc_temp_codigo = $codigo;}
;
