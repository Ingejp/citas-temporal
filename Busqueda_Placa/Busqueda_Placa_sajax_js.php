  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_Busqueda_Placa" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_Busqueda_Placa_form();
	}

  // ---------- Validate placa
  function do_ajax_Busqueda_Placa_validate_placa()
  {
  } // do_ajax_Busqueda_Placa_validate_placa

  function do_ajax_Busqueda_Placa_validate_placa_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_Busqueda_Placa_validate_placa_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_Busqueda_Placa_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_Busqueda_Placa_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_Busqueda_Placa_submit_form(var_placa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_Busqueda_Placa_submit_form_cb);
  } // do_ajax_Busqueda_Placa_submit_form

  function do_ajax_Busqueda_Placa_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_Busqueda_Placa_submit_form_cb_after_alert);
  } // do_ajax_Busqueda_Placa_submit_form_cb
  function do_ajax_Busqueda_Placa_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_Busqueda_Placa_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_Busqueda_Placa_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_Busqueda_Placa_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['Busqueda_Placa']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_Busqueda_Placa_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_Busqueda_Placa_navigate_form_cb_after_alert);
  } // do_ajax_Busqueda_Placa_navigate_form_cb
  function do_ajax_Busqueda_Placa_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_Busqueda_Placa_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_Busqueda_Placa_navigate_form_cb_after_alert
  function sc_hide_Busqueda_Placa_form()
  {
