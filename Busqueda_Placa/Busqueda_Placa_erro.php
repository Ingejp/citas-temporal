...
$old_error_handler = set_error_handler("Busqueda_Placa_scriptcase_error_handler");

function Busqueda_Placa_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['Busqueda_Placa']['contr_erro']) && $_SESSION['scriptcase']['Busqueda_Placa']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_Busqueda_Placa->contr_Busqueda_Placa->Apl_com_erro = true;
            if ($inicial_Busqueda_Placa->contr_Busqueda_Placa->NM_ajax_flag)
            {
                $inicial_Busqueda_Placa->contr_Busqueda_Placa->NM_ajax_info['errList']['geral_Busqueda_Placa'][] = $msg;
                return;
            }
            Busqueda_Placa_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function Busqueda_Placa_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['Busqueda_Placa'])  && '' != $_SESSION['scriptcase']['error_icon']['Busqueda_Placa'])  ? $_SESSION['scriptcase']['error_icon']['Busqueda_Placa']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['Busqueda_Placa']) && '' != $_SESSION['scriptcase']['error_close']['Busqueda_Placa']) ? $_SESSION['scriptcase']['error_close']['Busqueda_Placa'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['Busqueda_Placa']) && '' != $_SESSION['scriptcase']['error_span_title']['Busqueda_Placa']) ? $_SESSION['scriptcase']['error_span_title']['Busqueda_Placa'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['Busqueda_Placa']) && '' != $_SESSION['scriptcase']['error_icon_title']['Busqueda_Placa']) ? $_SESSION['scriptcase']['error_icon_title']['Busqueda_Placa'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['Busqueda_Placa']['error_buffer'] .= ob_get_flush();
}
