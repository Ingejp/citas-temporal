...
$old_error_handler = set_error_handler("VER_DISPONIBILIDAD_scriptcase_error_handler");

function VER_DISPONIBILIDAD_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['VER_DISPONIBILIDAD']['contr_erro']) && $_SESSION['scriptcase']['VER_DISPONIBILIDAD']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_VER_DISPONIBILIDAD->contr_VER_DISPONIBILIDAD->Apl_com_erro = true;
            if ($inicial_VER_DISPONIBILIDAD->contr_VER_DISPONIBILIDAD->NM_ajax_flag)
            {
                $inicial_VER_DISPONIBILIDAD->contr_VER_DISPONIBILIDAD->NM_ajax_info['errList']['geral_VER_DISPONIBILIDAD'][] = $msg;
                return;
            }
            VER_DISPONIBILIDAD_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function VER_DISPONIBILIDAD_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['VER_DISPONIBILIDAD'])  && '' != $_SESSION['scriptcase']['error_icon']['VER_DISPONIBILIDAD'])  ? $_SESSION['scriptcase']['error_icon']['VER_DISPONIBILIDAD']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['VER_DISPONIBILIDAD']) && '' != $_SESSION['scriptcase']['error_close']['VER_DISPONIBILIDAD']) ? $_SESSION['scriptcase']['error_close']['VER_DISPONIBILIDAD'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['VER_DISPONIBILIDAD']) && '' != $_SESSION['scriptcase']['error_span_title']['VER_DISPONIBILIDAD']) ? $_SESSION['scriptcase']['error_span_title']['VER_DISPONIBILIDAD'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['VER_DISPONIBILIDAD']) && '' != $_SESSION['scriptcase']['error_icon_title']['VER_DISPONIBILIDAD']) ? $_SESSION['scriptcase']['error_icon_title']['VER_DISPONIBILIDAD'] : '';
?>
