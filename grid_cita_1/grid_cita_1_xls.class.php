...
class grid_cita_1_xls
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "grid_cita_1_res_xls.class.php");
              $this->Res_xls = new grid_cita_1_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_cita_1.zip";
          $this->Arquivo   .= "_grid_cita_1" . $this->Xls_tp;
          $this->Tit_doc    = "grid_cita_1" . $this->Xls_tp;
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['field_order'] as $Cada_cmp)
      {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida'])
         { 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "cita_idbuque" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              if (!NM_is_utf8($SC_Label))
          ...
          if ($Cada_col == "pil_dpi" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              if (!NM_is_utf8($SC_Label))
   ...
   function NM_export_cita_idbuque()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         if (!NM_is_utf8($this->look_cita_idbuque))
         {
             $this->look_cita_idbuque = sc_convert_encoding($this->look_cita_idbuque, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if (is_numeric($this->look_cita_idbuque))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->look_cita_idbuque);
         $this->Xls_col++;
   }
   //----- cita_idestado
   ...
   function NM_export_pil_dpi()
   {
         ...
         $this->NM_ctrl_style[$current_cell_ref]['end'] = $this->Xls_row;
         if (!NM_is_utf8($this->pil_dpi))
         {
             $this->pil_dpi = sc_convert_encoding($this->pil_dpi, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if (is_numeric($this->pil_dpi))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->pil_dpi);
         $this->Xls_col++;
   }
   //----- cita_descripcion
   ...
   //----- cita_idbuque
   function NM_sub_cons_cita_idbuque()
   {
         if (!NM_is_utf8($this->look_cita_idbuque))
         {
             $this->look_cita_idbuque = sc_convert_encoding($this->look_cita_idbuque, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->look_cita_idbuque;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   //----- pil_dpi
   function NM_sub_cons_pil_dpi()
   {
         if (!NM_is_utf8($this->pil_dpi))
         {
             $this->pil_dpi = sc_convert_encoding($this->pil_dpi, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->pil_dpi;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "center";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['nolabel'])
       {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE>CITAS SOLICITADAS :: Excel</TITLE>
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cita_1']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
