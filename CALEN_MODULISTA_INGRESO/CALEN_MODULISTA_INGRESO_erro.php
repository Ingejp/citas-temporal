...
$old_error_handler = set_error_handler("CALEN_MODULISTA_INGRESO_scriptcase_error_handler");

function CALEN_MODULISTA_INGRESO_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['CALEN_MODULISTA_INGRESO']['contr_erro']) && $_SESSION['scriptcase']['CALEN_MODULISTA_INGRESO']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_CALEN_MODULISTA_INGRESO->contr_CALEN_MODULISTA_INGRESO->Apl_com_erro = true;
            if ($inicial_CALEN_MODULISTA_INGRESO->contr_CALEN_MODULISTA_INGRESO->NM_ajax_flag)
            {
                $inicial_CALEN_MODULISTA_INGRESO->contr_CALEN_MODULISTA_INGRESO->NM_ajax_info['errList']['geral_CALEN_MODULISTA_INGRESO'][] = $msg;
                return;
            }
            CALEN_MODULISTA_INGRESO_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function CALEN_MODULISTA_INGRESO_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['CALEN_MODULISTA_INGRESO'])  && '' != $_SESSION['scriptcase']['error_icon']['CALEN_MODULISTA_INGRESO'])  ? $_SESSION['scriptcase']['error_icon']['CALEN_MODULISTA_INGRESO']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['CALEN_MODULISTA_INGRESO']) && '' != $_SESSION['scriptcase']['error_close']['CALEN_MODULISTA_INGRESO']) ? $_SESSION['scriptcase']['error_close']['CALEN_MODULISTA_INGRESO'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['CALEN_MODULISTA_INGRESO']) && '' != $_SESSION['scriptcase']['error_span_title']['CALEN_MODULISTA_INGRESO']) ? $_SESSION['scriptcase']['error_span_title']['CALEN_MODULISTA_INGRESO'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['CALEN_MODULISTA_INGRESO']) && '' != $_SESSION['scriptcase']['error_icon_title']['CALEN_MODULISTA_INGRESO']) ? $_SESSION['scriptcase']['error_icon_title']['CALEN_MODULISTA_INGRESO'] : '';
?>
