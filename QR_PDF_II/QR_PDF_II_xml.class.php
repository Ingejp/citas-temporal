...
class QR_PDF_II_xml
{
   ...
   function inicializa_vars()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['embutida'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_label']))
      {
          $this->Xml_tag_label = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_label'];
      ...
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['SC_Ind_Groupby'];
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_return']);
          if ($this->Tem_xml_res) {
      ...
      $this->Arquivo     .= "_QR_PDF_II";
      $this->Arquivo_view = $this->Arquivo . "_view.xml";
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "QR_PDF_II.xml";
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_name']);
      }
      if (!$this->Grava_view)
      {
...
$_SESSION['scriptcase']['QR_PDF_II']['contr_erro'] = 'off'; 
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<QR_PDF_II>\r\n";
         }
         else
         {
             $this->xml_registro = "<QR_PDF_II";
      ...
      { 
          ...
          { 
              ...
              $this->Res = new QR_PDF_II_res_xml();
              $this->prep_modulos("Res");
              $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_grid'] = true;
              $this->Res->monta_xml();
          } 
          if (!$this->Ini->sc_export_ajax) {
          ...
          { 
              ...
              { 
                  $str_zip   = "";
                  $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_file']['xml'];
                  ...
                  unlink($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_file']['xml']);
              }
              if ($this->Grava_view)
              {
                  $str_zip    = "";
                  ...
                  { 
                      $str_zip   = "";
                      $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_file']['view'];
                      $Arq_input = (FALSE !== strpos($Arq_res, ' ')) ? " \"" . $Arq_res . "\"" :  $Arq_res;
                      if (FALSE !== strpos(strtolower(php_uname()), 'windows')) 
                      {
                          $str_zip = "zip.exe " . strtoupper($Parm_pass) . " -j -u " . $this->Xml_password . " " . $Zip_f . " " . $Arq_input;
                      ...
                      unlink($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_file']['view']);
                  }
              } 
              else 
              {
              ...
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_res_grid']);
          } 
      }
   ...
   function NM_export_idtipodecarga()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_idtipodecarga))
         {
          ...
          {
              $SC_Label = "idtipodecarga"; 
          }
          $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->look_idtipodecarga) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->look_idtipodecarga) . "\"";
         }
   }
   //----- idestado
   ...
   function NM_export_idselectivo()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_idselectivo))
         {
          ...
          {
              $SC_Label = "idselectivo"; 
          }
          $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->look_idselectivo) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->look_idselectivo) . "\"";
         }
   }
   //----- idyarda
   function NM_export_idyarda()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->idyarda) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->idyarda) . "\"";
         }
   }
   //----- idtipomovimiento
   function NM_export_idtipomovimiento()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->look_idtipomovimiento))
         {
          ...
          {
              $SC_Label = "idtipomovimiento"; 
          }
          $this->clear_tag($SC_Label); 
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->look_idtipomovimiento) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->look_idtipomovimiento) . "\"";
         }
   }
   //----- idviaje
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_chart_title'] ?> cita :: XML</TITLE>
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['QR_PDF_II']['xml_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
