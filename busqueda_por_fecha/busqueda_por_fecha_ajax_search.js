...
function ajax_ch_bi_search(field, parm)
{
    ...
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_ch_bi_search&script_case_init=" + document.F1.script_case_init.value + "&script_case_session=" + document.F1.script_case_session.value + "&Campo_bi=" + field + "&Opc_bi=" + parm
     })
...
Table_sv_fil[0] = "fechainicio";
Table_sv_fil[1] = "naviera";
function ajax_select_filter(parm)
{
    ...
    $.ajax({
     ...
     .done(function(json_sel_fil) {
        ...
        if ($("#SC_fechainicio_cond") && search_get_sel_txt("SC_fechainicio_cond").substring(0, 3) == "bi_") {
            formata_bi_fechainicio(search_get_sel_txt("SC_fechainicio_cond"), SC_fechainicio_bi_opc, SC_fechainicio_bi_dt);
        }
