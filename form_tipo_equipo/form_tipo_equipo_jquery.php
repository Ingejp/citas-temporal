...
function scEventControl_init(iSeqRow) {
  scEventControl_data["idtipo_equipo_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["descripcion_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["idtipo_equipo_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["idtipo_equipo_" + iSeqRow]["change"]) {
    return true;
  }
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_idtipo_equipo_' + iSeqRow).bind('blur', function() { sc_form_tipo_equipo_idtipo_equipo__onblur(this, iSeqRow) })
                                            .bind('change', function() { sc_form_tipo_equipo_idtipo_equipo__onchange(this, iSeqRow) })
                                            .bind('focus', function() { sc_form_tipo_equipo_idtipo_equipo__onfocus(this, iSeqRow) });
  $('#id_sc_field_descripcion_' + iSeqRow).bind('blur', function() { sc_form_tipo_equipo_descripcion__onblur(this, iSeqRow) })
                                          .bind('change', function() { sc_form_tipo_equipo_descripcion__onchange(this, iSeqRow) })
                                          .bind('focus', function() { sc_form_tipo_equipo_descripcion__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_tipo_equipo_idtipo_equipo__onblur(oThis, iSeqRow) {
  do_ajax_form_tipo_equipo_validate_idtipo_equipo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tipo_equipo_idtipo_equipo__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_tipo_equipo_idtipo_equipo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_tipo_equipo_descripcion__onblur(oThis, iSeqRow) {
  do_ajax_form_tipo_equipo_validate_descripcion_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_tipo_equipo_descripcion__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_tipo_equipo_descripcion__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("idtipo_equipo_", "", status);
	displayChange_field("descripcion_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_idtipo_equipo_(row, status);
	displayChange_field_descripcion_(row, status);
}

function displayChange_field(field, row, status) {
	if ("idtipo_equipo_" == field) {
		displayChange_field_idtipo_equipo_(row, status);
	}
...
function displayChange_field_idtipo_equipo_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_tipo_equipo_form" + pageNo).hide();
}
