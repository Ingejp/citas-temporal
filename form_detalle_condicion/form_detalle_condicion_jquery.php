...
function scEventControl_init(iSeqRow) {
  scEventControl_data["iddetalle_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["foto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["iddetalle_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["iddetalle_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scEventControl_onFocus(oField, iSeq) {
  ...
  if ("idcondicion_" + iSeq == fieldName) {
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_iddetalle_' + iSeqRow).bind('blur', function() { sc_form_detalle_condicion_iddetalle__onblur(this, iSeqRow) })
                                        .bind('change', function() { sc_form_detalle_condicion_iddetalle__onchange(this, iSeqRow) })
                                        .bind('focus', function() { sc_form_detalle_condicion_iddetalle__onfocus(this, iSeqRow) });
  $('#id_sc_field_foto_' + iSeqRow).bind('blur', function() { sc_form_detalle_condicion_foto__onblur(this, iSeqRow) })
                                   .bind('change', function() { sc_form_detalle_condicion_foto__onchange(this, iSeqRow) })
                                   .bind('focus', function() { sc_form_detalle_condicion_foto__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_detalle_condicion_iddetalle__onblur(oThis, iSeqRow) {
  do_ajax_form_detalle_condicion_validate_iddetalle_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_detalle_condicion_iddetalle__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_detalle_condicion_iddetalle__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_detalle_condicion_foto__onblur(oThis, iSeqRow) {
  scCssBlur(oThis, iSeqRow);
}

function sc_form_detalle_condicion_foto__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_detalle_condicion_foto__onfocus(oThis, iSeqRow) {
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("iddetalle_", "", status);
	displayChange_field("foto_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_iddetalle_(row, status);
	displayChange_field_foto_(row, status);
}
...
function displayChange_field_iddetalle_(row, status) {
}

function displayChange_field_foto_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_detalle_condicion_form" + pageNo).hide();
}
...
function scJQUploadAdd(iSeqRow) {
  $("#id_sc_field_foto_" + iSeqRow).fileupload({
    datatype: "json",
    url: "form_detalle_condicion_ul_save.php",
    dropZone: "",
    formData: function() {
      return [
        {name: 'param_field', value: 'foto_'},
    ...
    progress: function(e, data) {
      ...
      $("#id_sc_field_foto__ul_name" + iSeqRow).val(fileData[0].sc_ul_name);
      $("#id_sc_field_foto__ul_type" + iSeqRow).val(fileData[0].type);
      eval("var_ajax_img_foto_" + iSeqRow + " = '<?php echo $this->Ini->path_imag_temp; ?>/' + fileData[0].sc_image_source");
      var_ajax_img_foto_ = '<?php echo $this->Ini->path_imag_temp; ?>/' + fileData[0].sc_image_source;
      ...
      $("#txt_ajax_img_foto_" + iSeqRow).html(fileData[0].name);
      $("#txt_ajax_img_foto_" + iSeqRow).css("display", "none");
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "idcondicion_" == specificField) {
    scJQSelect2Add_idcondicion_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_idcondicion_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_idcondicion__obj" : "#id_sc_field_idcondicion_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_idcondicion__obj',
      dropdownCssClass: 'css_idcondicion__obj',
      language: {
        noResults: function() {
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_idcondicion_) { displayChange_field_idcondicion_(iLine, "on"); } }, 150);
} // scJQElementsAdd
