  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_SUBIR_EXCEL_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_SUBIR_EXCEL_mob_form();
	}

  // ---------- Validate importar
  function do_ajax_SUBIR_EXCEL_mob_validate_importar()
  {
    var nomeCampo_importar = "importar";
    var var_importar = scAjaxGetFieldText(nomeCampo_importar);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_SUBIR_EXCEL_mob_validate_importar(var_importar, var_script_case_init, do_ajax_SUBIR_EXCEL_mob_validate_importar_cb);
  } // do_ajax_SUBIR_EXCEL_mob_validate_importar

  function do_ajax_SUBIR_EXCEL_mob_validate_importar_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "importar";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_SUBIR_EXCEL_mob_validate_importar_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_SUBIR_EXCEL_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_SUBIR_EXCEL_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_importar = scAjaxGetFieldText("importar");
    var var_importar_ul_name = scAjaxSpecCharProtect(document.F1.importar_ul_name.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_importar_ul_type = document.F1.importar_ul_type.value;
    var var_importar_salva = scAjaxSpecCharProtect(document.F1.importar_salva.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_importar_limpa = document.F1.importar_limpa.checked ? "S" : "N";
    ...
    scAjaxProcOn();
    x_ajax_SUBIR_EXCEL_mob_submit_form(var_importar, var_importar_ul_name, var_importar_ul_type, var_importar_salva, var_importar_limpa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_SUBIR_EXCEL_mob_submit_form_cb);
  } // do_ajax_SUBIR_EXCEL_mob_submit_form

  function do_ajax_SUBIR_EXCEL_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    if (!scAjaxHasError())
    {
      ...
      if (document.F1.importar_limpa && document.F1.importar_limpa.checked)
      {
        var oImgTemp = {0: {"value" : ""}};
        scAjaxSetFieldDocument("importar", oImgTemp, "", "");
      }
      else if (document.getElementById("id_ajax_doc_importar") && "" != document.getElementById("id_ajax_doc_importar").innerHTML)
      {
        document.getElementById("id_ajax_doc_importar").style.display = "";
      }
    }
    scAjaxShowDebug();
    scAjaxSetDisplay();
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_SUBIR_EXCEL_mob_submit_form_cb_after_alert);
  } // do_ajax_SUBIR_EXCEL_mob_submit_form_cb
  function do_ajax_SUBIR_EXCEL_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_SUBIR_EXCEL_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_SUBIR_EXCEL_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_SUBIR_EXCEL_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['SUBIR_EXCEL_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_SUBIR_EXCEL_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_SUBIR_EXCEL_mob_navigate_form_cb_after_alert);
  } // do_ajax_SUBIR_EXCEL_mob_navigate_form_cb
  function do_ajax_SUBIR_EXCEL_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_SUBIR_EXCEL_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_SUBIR_EXCEL_mob_navigate_form_cb_after_alert
  function sc_hide_SUBIR_EXCEL_mob_form()
  {
  ...
  ajax_field_list[0] = "importar";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "importar": {"label": "IMPORTAR", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["importar"][1] = "importar";

  var ajax_field_id = {
    "importar": new Array("hidden_field_label_importar", "hidden_field_data_importar")
  };
