...
$old_error_handler = set_error_handler("SUBIR_EXCEL_scriptcase_error_handler");

function SUBIR_EXCEL_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['SUBIR_EXCEL']['contr_erro']) && $_SESSION['scriptcase']['SUBIR_EXCEL']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_SUBIR_EXCEL->contr_SUBIR_EXCEL->Apl_com_erro = true;
            if ($inicial_SUBIR_EXCEL->contr_SUBIR_EXCEL->NM_ajax_flag)
            {
                $inicial_SUBIR_EXCEL->contr_SUBIR_EXCEL->NM_ajax_info['errList']['geral_SUBIR_EXCEL'][] = $msg;
                return;
            }
            SUBIR_EXCEL_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function SUBIR_EXCEL_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['SUBIR_EXCEL'])  && '' != $_SESSION['scriptcase']['error_icon']['SUBIR_EXCEL'])  ? $_SESSION['scriptcase']['error_icon']['SUBIR_EXCEL']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['SUBIR_EXCEL']) && '' != $_SESSION['scriptcase']['error_close']['SUBIR_EXCEL']) ? $_SESSION['scriptcase']['error_close']['SUBIR_EXCEL'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['SUBIR_EXCEL']) && '' != $_SESSION['scriptcase']['error_span_title']['SUBIR_EXCEL']) ? $_SESSION['scriptcase']['error_span_title']['SUBIR_EXCEL'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['SUBIR_EXCEL']) && '' != $_SESSION['scriptcase']['error_icon_title']['SUBIR_EXCEL']) ? $_SESSION['scriptcase']['error_icon_title']['SUBIR_EXCEL'] : '';
?>
