...
$old_error_handler = set_error_handler("registro_turnos_mob_scriptcase_error_handler");

function registro_turnos_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['registro_turnos_mob']['contr_erro']) && $_SESSION['scriptcase']['registro_turnos_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_registro_turnos_mob->contr_registro_turnos_mob->Apl_com_erro = true;
            if ($inicial_registro_turnos_mob->contr_registro_turnos_mob->NM_ajax_flag)
            {
                $inicial_registro_turnos_mob->contr_registro_turnos_mob->NM_ajax_info['errList']['geral_registro_turnos_mob'][] = $msg;
                return;
            }
            registro_turnos_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function registro_turnos_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['registro_turnos_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['registro_turnos_mob'])  ? $_SESSION['scriptcase']['error_icon']['registro_turnos_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['registro_turnos_mob']) && '' != $_SESSION['scriptcase']['error_close']['registro_turnos_mob']) ? $_SESSION['scriptcase']['error_close']['registro_turnos_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['registro_turnos_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['registro_turnos_mob']) ? $_SESSION['scriptcase']['error_span_title']['registro_turnos_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['registro_turnos_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['registro_turnos_mob']) ? $_SESSION['scriptcase']['error_icon_title']['registro_turnos_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['registro_turnos']['error_buffer'] .= ob_get_flush();
}
