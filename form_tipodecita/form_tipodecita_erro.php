...
$old_error_handler = set_error_handler("form_tipodecita_scriptcase_error_handler");

function form_tipodecita_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_tipodecita']['contr_erro']) && $_SESSION['scriptcase']['form_tipodecita']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_tipodecita->contr_form_tipodecita->Apl_com_erro = true;
            if ($inicial_form_tipodecita->contr_form_tipodecita->NM_ajax_flag)
            {
                $inicial_form_tipodecita->contr_form_tipodecita->NM_ajax_info['errList']['geral_form_tipodecita'][] = $msg;
                return;
            }
            form_tipodecita_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_tipodecita_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_tipodecita'])  && '' != $_SESSION['scriptcase']['error_icon']['form_tipodecita'])  ? $_SESSION['scriptcase']['error_icon']['form_tipodecita']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_tipodecita']) && '' != $_SESSION['scriptcase']['error_close']['form_tipodecita']) ? $_SESSION['scriptcase']['error_close']['form_tipodecita'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_tipodecita']) && '' != $_SESSION['scriptcase']['error_span_title']['form_tipodecita']) ? $_SESSION['scriptcase']['error_span_title']['form_tipodecita'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_tipodecita']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_tipodecita']) ? $_SESSION['scriptcase']['error_icon_title']['form_tipodecita'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_tipodecita']['error_buffer'] .= ob_get_flush();
}
