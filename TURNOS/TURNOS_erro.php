...
$old_error_handler = set_error_handler("TURNOS_scriptcase_error_handler");

function TURNOS_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['TURNOS']['contr_erro']) && $_SESSION['scriptcase']['TURNOS']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_TURNOS->contr_TURNOS->Apl_com_erro = true;
            if ($inicial_TURNOS->contr_TURNOS->NM_ajax_flag)
            {
                $inicial_TURNOS->contr_TURNOS->NM_ajax_info['errList']['geral_TURNOS'][] = $msg;
                return;
            }
            TURNOS_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function TURNOS_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['TURNOS'])  && '' != $_SESSION['scriptcase']['error_icon']['TURNOS'])  ? $_SESSION['scriptcase']['error_icon']['TURNOS']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['TURNOS']) && '' != $_SESSION['scriptcase']['error_close']['TURNOS']) ? $_SESSION['scriptcase']['error_close']['TURNOS'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['TURNOS']) && '' != $_SESSION['scriptcase']['error_span_title']['TURNOS']) ? $_SESSION['scriptcase']['error_span_title']['TURNOS'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['TURNOS']) && '' != $_SESSION['scriptcase']['error_icon_title']['TURNOS']) ? $_SESSION['scriptcase']['error_icon_title']['TURNOS'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['TURNOS']['error_buffer'] .= ob_get_flush();
}
