...
$old_error_handler = set_error_handler("sec_logged_mob_scriptcase_error_handler");

function sec_logged_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['sec_logged_mob']['contr_erro']) && $_SESSION['scriptcase']['sec_logged_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sec_logged_mob->contr_sec_logged_mob->Apl_com_erro = true;
            if ($inicial_sec_logged_mob->contr_sec_logged_mob->NM_ajax_flag)
            {
                $inicial_sec_logged_mob->contr_sec_logged_mob->NM_ajax_info['errList']['geral_sec_logged_mob'][] = $msg;
                return;
            }
            sec_logged_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sec_logged_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sec_logged_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['sec_logged_mob'])  ? $_SESSION['scriptcase']['error_icon']['sec_logged_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sec_logged_mob']) && '' != $_SESSION['scriptcase']['error_close']['sec_logged_mob']) ? $_SESSION['scriptcase']['error_close']['sec_logged_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sec_logged_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['sec_logged_mob']) ? $_SESSION['scriptcase']['error_span_title']['sec_logged_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sec_logged_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['sec_logged_mob']) ? $_SESSION['scriptcase']['error_icon_title']['sec_logged_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['sec_logged']['error_buffer'] .= ob_get_flush();
}
