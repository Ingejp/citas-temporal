...
$old_error_handler = set_error_handler("form_tipodecarga_scriptcase_error_handler");

function form_tipodecarga_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_tipodecarga']['contr_erro']) && $_SESSION['scriptcase']['form_tipodecarga']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_tipodecarga->contr_form_tipodecarga->Apl_com_erro = true;
            if ($inicial_form_tipodecarga->contr_form_tipodecarga->NM_ajax_flag)
            {
                $inicial_form_tipodecarga->contr_form_tipodecarga->NM_ajax_info['errList']['geral_form_tipodecarga'][] = $msg;
                return;
            }
            form_tipodecarga_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_tipodecarga_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_tipodecarga'])  && '' != $_SESSION['scriptcase']['error_icon']['form_tipodecarga'])  ? $_SESSION['scriptcase']['error_icon']['form_tipodecarga']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_tipodecarga']) && '' != $_SESSION['scriptcase']['error_close']['form_tipodecarga']) ? $_SESSION['scriptcase']['error_close']['form_tipodecarga'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_tipodecarga']) && '' != $_SESSION['scriptcase']['error_span_title']['form_tipodecarga']) ? $_SESSION['scriptcase']['error_span_title']['form_tipodecarga'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_tipodecarga']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_tipodecarga']) ? $_SESSION['scriptcase']['error_icon_title']['form_tipodecarga'] : '';
?>
