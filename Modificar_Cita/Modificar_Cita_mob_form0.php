...
.sc-button-image.disabled img {
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>Modificar_Cita/Modificar_Cita_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("Modificar_Cita_mob_sajax_js.php");
?>
<script type="text/javascript">
...
include_once('Modificar_Cita_mob_jquery.php');
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['recarga'];
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
?>
<body class="scFormPage" style="<?php echo $remove_margin . $str_iframe_body; ?>">
<?php

if (isset($_SESSION['scriptcase']['Modificar_Cita']['error_buffer']) && '' != $_SESSION['scriptcase']['Modificar_Cita']['error_buffer'])
{
    echo $_SESSION['scriptcase']['Modificar_Cita']['error_buffer'];
}
...
<?php
 include_once("Modificar_Cita_mob_js0.php");
?>
<script type="text/javascript"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['Modificar_Cita_mob'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['Modificar_Cita_mob'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['mostra_cab'] != "N") && (!$_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['under_dashboard'] || !$_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['compact_mode'] || $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['maximized']))
  {
?>
<tr><td>
<style>
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['empty_filter'] = true;
       }
  }
?>
...
?>
   ...
   <?php
    ...
    {
        $this->nm_new_label['codigo_cita'] = " CODIGO";
    }
?>
<?php
   $nm_cor_fun_cel  = ($nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['coment']))
    {
        $this->nm_new_label['coment'] = "COMENT";
    }
?>
<?php
   $nm_cor_fun_cel  = ($nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   ...
   $coment = $this->coment;
   $sStyleHidden_coment = '';
   if (isset($this->nmgp_cmp_hidden['coment']) && $this->nmgp_cmp_hidden['coment'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['coment']);
       $sStyleHidden_coment = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_coment = 'display: none;';
   $sStyleReadInp_coment = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['coment']) && $this->nmgp_cmp_readonly['coment'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['coment']);
       $sStyleReadLab_coment = '';
       $sStyleReadInp_coment = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['coment']) && $this->nmgp_cmp_hidden['coment'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="coment" value="<?php echo $this->form_encode_input($coment) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_coment_line" id="hidden_field_data_coment" style="<?php echo $sStyleHidden_coment; ?>"> <span class="scFormLabelOddFormat css_coment_label"><span id="id_label_coment"><?php echo $this->nm_new_label['coment']; ?></span></span><br>
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["coment"]) &&  $this->nmgp_cmp_readonly["coment"] == "on") { 

 ?>
<input type="hidden" name="coment" value="<?php echo $this->form_encode_input($coment) . "\">" . $coment . ""; ?>
<?php } else { ?>
<span id="id_read_on_coment" class="sc-ui-readonly-coment css_coment_line" style="<?php echo $sStyleReadLab_coment; ?>"><?php echo $this->coment; ?></span><span id="id_read_off_coment" class="css_read_off_coment" style="white-space: nowrap;<?php echo $sStyleReadInp_coment; ?>">
 <input class="sc-js-input scFormObjectOdd css_coment_obj" style="" id="id_sc_field_coment" type=text name="coment" value="<?php echo $this->form_encode_input($coment) ?>"
...
<?php
           if (($nm_apl_dependente != 1) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['under_dashboard'])) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['nm_run_menu']) || $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['nm_run_menu'] != 1))) {
...
<?php
           if (($nm_apl_dependente == 1) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['under_dashboard']))) {
        $sCondStyle = ($this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("Modificar_Cita_mob");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("Modificar_Cita_mob");
 parent.scAjaxDetailHeight("Modificar_Cita_mob", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
?>
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['Modificar_Cita_mob']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
