...
$old_error_handler = set_error_handler("Modificar_Cita_scriptcase_error_handler");

function Modificar_Cita_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['Modificar_Cita']['contr_erro']) && $_SESSION['scriptcase']['Modificar_Cita']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_Modificar_Cita->contr_Modificar_Cita->Apl_com_erro = true;
            if ($inicial_Modificar_Cita->contr_Modificar_Cita->NM_ajax_flag)
            {
                $inicial_Modificar_Cita->contr_Modificar_Cita->NM_ajax_info['errList']['geral_Modificar_Cita'][] = $msg;
                return;
            }
            Modificar_Cita_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function Modificar_Cita_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['Modificar_Cita'])  && '' != $_SESSION['scriptcase']['error_icon']['Modificar_Cita'])  ? $_SESSION['scriptcase']['error_icon']['Modificar_Cita']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['Modificar_Cita']) && '' != $_SESSION['scriptcase']['error_close']['Modificar_Cita']) ? $_SESSION['scriptcase']['error_close']['Modificar_Cita'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['Modificar_Cita']) && '' != $_SESSION['scriptcase']['error_span_title']['Modificar_Cita']) ? $_SESSION['scriptcase']['error_span_title']['Modificar_Cita'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['Modificar_Cita']) && '' != $_SESSION['scriptcase']['error_icon_title']['Modificar_Cita']) ? $_SESSION['scriptcase']['error_icon_title']['Modificar_Cita'] : '';
?>
